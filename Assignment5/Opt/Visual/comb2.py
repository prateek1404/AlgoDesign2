...
def arrayOfSetsOf(previous,n,k):
   A={}
   setOfSize={}
   A={}
   setOfSize=[]
...
for m in range(0,count):
 ...
 setOfSize,previous = arrayOfSetsOf(previous,count,m)
...
for b in range(1,count):
 ...
 # print dis
  if TSP[arrayS][b]+dis<ans:
   lastPoint =b
   distanceTillLast = TSP[arrayS][b]
   ans = TSP[arrayS][b]+dis
print ans
print pointArray[lastPoint]
print distanceTillLast
fromLastToB = ((float(pointArray[lastPoint]['x'])-23883.3333)**2 + (float(pointArray[lastPoint]['y'])-14533.3333)**2)**0.5
print distanceTillLast + fromLastToB
